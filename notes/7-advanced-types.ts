...
// or let x: typeof Promise.resolve;
...
let a: EventualType<Promise<number>>; // number
let b: EventualType<number[]>; // number[]
...
type HasThen<T> = Pick<Promise<T>, 'then' | 'catch'>; // catch, then
// type HasThen<T> = Promise<T>; // Symbol, catch, finally, then

let hasThen: HasThen<number> = Promise.resolve(4);
hasThen.then;
hasThen.catch;
hasThen.finally; // error
...
let onlystrings: OnlyStrings; // type 'a' | 'b';
onlystrings = 'a';
onlystrings = 'josh'; // error: type 'josh' is not assignable to type 'a' | 'b'
onlystrings = 3; // error: type 3 is not assignable to type 1 | 2
...
let notstrings: NotStrings; // type 1 | 2
notstrings = 1;
notstrings = 'a'; // error: not assignable to type 1 | 2
